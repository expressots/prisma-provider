    ...
    if (sourceFile) {
      ts.forEachChild(sourceFile, node => {
        ...
        if (ts.isTypeAliasDeclaration(node)) {
          ...
          fileModule.processProperty(typeObject, node, sourceFile, (propertyName: string, propertyType: string, pathDeclaration: string) => {
            typeProperties.push({ name: propertyName, type: propertyType, pathDeclaration });
          });
        ...
        if (ts.isClassDeclaration(node)) {
          ...
          fileModule.processProperty(classObject, node, sourceFile, (propertyName: string, propertyType: string, pathDeclaration: string) => {
            classProperties.push({ name: propertyName, type: propertyType, pathDeclaration });
          });
